...
file_section <- function(template_name, template) {
  ...
  column_lines <- unlist(lapply(columns, function(column) {
    column_schema <- schema$columns[schema$columns$column == column, , drop = FALSE]
    if (nrow(column_schema) == 0) {
      explanation <- "Generated package column. Keep unchanged unless you know this field is wrong."
      requirement <- "generated"
      validation <- "none"
      allowed_values <- ""
    } else {
      explanation <- column_schema$description[[1]]
      requirement <- column_schema$requirement[[1]]
      validation <- column_schema$validation_type[[1]]
      allowed_values <- column_schema$allowed_values[[1]]
    }
    marker <- if (requirement %in% c("required", "conditional")) " *" else ""
    validation_line <- paste0("Validation: ", validation, ".")
    if (nzchar(allowed_values)) {
      validation_line <- paste0(validation_line, " Allowed values: ", allowed_values, ".")
    }
  ...
  c(
    ...
    "Common checks:",
    "",
    paste0("- ", paste(schema$checks, collapse = "\n- ")),
    ""
  )
}
...
template_file_dictionary <- function() {
  ...
  list(
    ...
    "04_life_expectancy" = list(
      ...
      rows = "One row per age start, sex, and stratum combination.",
      columns = list(
    ...
    "06_dismod_input_skeleton" = list(
      purpose = "Provides a long-format disease-consistency solver evidence skeleton. This can be filled directly or populated from `05_disease_epidemiology_raw.csv`.",
      rows = "One row per age, sex, stratum, disease, and solver evidence parameter.",
      columns = list(
        age_start = id_age,
        age_end = id_age_end,
        age_label = id_age_label,
    ...
    "11_stratum_rate_ratios" = list(
      ...
      rows = "One row per age, sex, stratum, and disaggregated parameter.",
      columns = list(
        age_start = id_age,
        age_end = id_age_end,
        age_label = id_age_label,
...
use_after_completion_section <- function() {
  c(
    ...
    "2. Run `diagnose_missing_parameters()` on the disease epidemiology file to see whether each disease has enough point estimates for deterministic consistency solving.",
    "3. Add any solver-grade long evidence to `06_dismod_input_skeleton.csv`, using `parameter = \"mortality\"` only for explicit disease-specific mortality evidence.",
    "4. Run `solve_disease_consistency()` to create `pmslt_disease_epi.csv`; use that solver output rather than the raw disease epidemiology file for PMSLT simulation inputs.",
